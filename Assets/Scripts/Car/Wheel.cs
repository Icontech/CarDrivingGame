...
public class Wheel : MonoBehaviour {
	...
	void Start () {
		...
		if(transform.tag == "WheelForward"){
			maxSteeringAngle = DataKeeper.dk.maxSteeringAngle;
		}
	}
	...
	void FixedUpdate () {
		...
		if (model != null)
		{
			...
			//Make the steering harder when below 3 degrees
			if(Mathf.Abs(steering*DataKeeper.dk.degreesOfRotation) < 3.0f)
				model.transform.localRotation = Quaternion.Euler (Mathf.Rad2Deg * rotation, (maxSteeringAngle-1) * steering, 0);
			else
